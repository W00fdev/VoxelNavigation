...
namespace Octrees
{
    ...
    public class Graph
    {
        ...
        public OctreeNode GetPathNode(int index)
        {
            ...
            {
                Debug.LogError(
                    $"#{Time.frameCount}: Index out of bounds. PathLength: {pathList.Count}, Index: {index}");
                return null;
            }
            ...
            while (openSet.Count > 0)
            {
                ...
                if (current.Equals(end))
                {
                    ...
                    UnityEngine.Debug.Log(
                        $"#{Time.frameCount}: path size -- {pathList.Count}; actionsTaken -- {actionsTaken}");
                    return true;
                }
                ...
                foreach (Edge edge in current.edges)
                {
                    ...
                    float tentative_gScore = current.g +
