...
namespace Fraktalia.VoxelGen
{
	...
	{
		public static readonly NativeVoxelNode NullRef = default(NativeVoxelNode);
		...
		public NativeVoxelNode(IntPtr nulladdress)
		{
			Children = nulladdress;
			Settings = 0;			
			Address = nulladdress;
			Depth = 0;
			_voxelID = 0;
		...
		public NativeVoxelNode(byte depth, byte index, int voxelID, IntPtr parent, IntPtr address, IntPtr nulladdress)
		{
			Children = nulladdress;
			Settings = 0;
		...
		public NativeVoxelNode GetChildFast(int index)
		{
			void* ptr = Children.ToPointer();
			//Even if Null, it will lead to the NullAddress returning a invalid Voxel > safe.
			return UnsafeUtility.ArrayElementAsRef<NativeVoxelNode>(ptr, index);		
		}
		...
		public void DestroyChildren(ref NativeVoxelTree octree)
		{
			if (HasChildren())
			{
				...
				Children = octree.voxelReservoir.NullAddress;
			}
		}

		public void DestroyChildrenIgnoreID_Iterative(ref NativeVoxelTree octree)
		{
			if (!HasChildren())
				return;

			var nodeQueue = octree.NodeQueue;

			// Use a stack to simulate recursion
			nodeQueue.Clear();
			nodeQueue.Enqueue(Children);

			while (nodeQueue.Count > 0)
			{
				IntPtr currentChildren = nodeQueue.Dequeue();
				void* childptr = currentChildren.ToPointer();
				int length = octree.NodeChildrenCount;
				...
				for (int i = 0; i < length; i++)
				{
					...
					if (voxel.HasChildren())
						nodeQueue.Enqueue(voxel.Children); // Push child nodes for further processing

					voxel.MarkNodeAsDestroyed();
					UnsafeUtility.WriteArrayElement(childptr, i, voxel);
				}

				// Add the current node’s children to garbage collection
				octree.voxelReservoir.AddGarbage(currentChildren);
			}

			MarkNodeAsLeaf(); // Mark the original node as a leaf after children are processed
		}
		...
		public bool FillChildren(ref NativeVoxelTree octree)
		{
			...
			for (int i = 0; i < length; i++)
			{
				...
				NativeVoxelNode newVoxel = new NativeVoxelNode((byte)(nextdepth), (byte)i, _voxelID, Address, location, octree.voxelReservoir.NullAddress);
